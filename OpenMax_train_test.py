...
import classifiers.classifier_images_openmax as classifier_openmax
...
if opt.gzsl:
    ...
    gzsl_cls_openmax = classifier_openmax.CLASSIFIER(gzsl_X, gzsl_Y, data, known_classes.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, alpha = opt.openmax_alpha, eta = opt.openmax_eta)
else:
    zsl_cls_openmax = classifier_openmax.CLASSIFIER(unseen_feature, util.map_label(unseen_label, data.unseenclasses), data, data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=False, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, alpha = opt.openmax_alpha, eta = opt.openmax_eta)
if opt.gzsl:
    print("gzsl_cls H is: ", gzsl_cls_openmax.H)
    print("gzsl_cls acc_seen is: ", gzsl_cls_openmax.acc_seen)
    print("gzsl_cls acc_unseen is: ", gzsl_cls_openmax.acc_unseen)
    print("gzsl_cls fpr95 is: ", gzsl_cls_openmax.fpr95)
    print("gzsl_cls auroc is: ", gzsl_cls_openmax.auroc)
else:
    print("zsl_cls_openmax acc : {}".format(zsl_cls_openmax.acc))
    print("zsl_cls_openmax fpr95 : {}".format(zsl_cls_openmax.fpr95))
    print("zsl_cls_openmax auroc : {}".format(zsl_cls_openmax.auroc))
