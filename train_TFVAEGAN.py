...
print("{} of training samples: ".format(data.ntrain))
...
noise = torch.FloatTensor(opt.batch_size, opt.nz) # 64 * 312
...
for epoch in range(0,opt.nepoch):
    for loop in range(0,opt.feedback_loop):
        for i in range(0, data.ntrain, opt.batch_size):
            ...
            if opt.recons_weight > 0 and not opt.freeze_dec: # not train decoder at feedback time
                optimizerDec.step() 
    print('[{:d}/{:d}]  Loss_D: {:.4f} Loss_G: {:.4f}, Wasserstein_dist:{:.4f}, vae_loss_seen:{:.4f}'.format(epoch, opt.nepoch, D_cost.data[0], G_cost.data[0], Wasserstein_D.data[0],vae_loss_seen.data[0]))
    netG.eval()
    netDec.eval()
    netF.eval()
    ...
    if opt.gzsl:
        model_save_dir = os.path.join(opt.model_dir, opt.dataset, 'gzsl')
        ...
        gzsl_cls = classifier.CLASSIFIER(train_X, util.map_label(train_Y, known_classes), data, known_classes.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, test_openset = False)
        if best_gzsl_H < gzsl_cls.H:
            best_acc_seen, best_acc_unseen, best_gzsl_H = gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H
            torch.save(netG.state_dict(), '{}/{}/gzsl/netG_best.pth'.format(opt.model_dir, opt.dataset))
            torch.save(netDec.state_dict(), '{}/{}/gzsl/netDec_best.pth'.format(opt.model_dir, opt.dataset))
            torch.save(netF.state_dict(), '{}/{}/gzsl/netF_best.pth'.format(opt.model_dir, opt.dataset))
            torch.save(netD.state_dict(), '{}/{}/gzsl/netD_best.pth'.format(opt.model_dir, opt.dataset))
            torch.save(netE.state_dict(), '{}/{}/gzsl/netE_best.pth'.format(opt.model_dir, opt.dataset))
            torch.save(gzsl_cls.best_model, '{}/{}/gzsl/cls_best.pth'.format(opt.model_dir, opt.dataset))
        print('GZSL: seen={:.4f}, unseen={:.4f}, h={:.4f}'.format(gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H))

    # Zero-shot learning
    # Train ZSL classifier
    model_save_dir = os.path.join(opt.model_dir, opt.dataset, 'zsl')
    if not os.path.exists(model_save_dir):
        os.makedirs(model_save_dir)   
    zsl_cls = classifier.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), \
                    ...
                    generalized=False, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, test_openset = False)
    acc = zsl_cls.acc
    if best_zsl_acc < acc:
        best_zsl_acc = acc
        #save model
        torch.save(netG.state_dict(), '{}/{}/zsl/netG_best.pth'.format(opt.model_dir, opt.dataset))
        torch.save(netDec.state_dict(), '{}/{}/zsl/netDec_best.pth'.format(opt.model_dir, opt.dataset))
        torch.save(netF.state_dict(), '{}/{}/zsl/netF_best.pth'.format(opt.model_dir, opt.dataset))
        torch.save(netD.state_dict(), '{}/{}/zsl/netD_best.pth'.format(opt.model_dir, opt.dataset))
        torch.save(netE.state_dict(), '{}/{}/zsl/netE_best.pth'.format(opt.model_dir, opt.dataset))
        torch.save(zsl_cls.best_model, '{}/{}/zsl/cls_best.pth'.format(opt.model_dir, opt.dataset))
    print('ZSL: unseen accuracy={:.4f}'.format(acc))
...
print('the best ZSL unseen accuracy is {}'.format(best_zsl_acc))
if opt.gzsl:
    print('Dataset {}'.format(opt.dataset))
    print('the best GZSL seen accuracy is {}'.format(best_acc_seen))
    print('the best GZSL unseen accuracy is {}'.format(best_acc_unseen))
    print('the best GZSL H is {}'.format(best_gzsl_H))
