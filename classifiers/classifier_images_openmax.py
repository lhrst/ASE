...
import copy
import pdb
import classifiers.openmax_utils as openmax_utils

class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, netDec=None, dec_size=4096, dec_hidden_size=4096, usesoftmax = True, test_openset = True, eta = 10, alpha = 10):
        ...
        self.alpha = min(int(alpha), self.nclass)
        self.eta = int(eta)
        ...
        test_in_set = torch.utils.data.TensorDataset(self.test_unseen_feature, self.test_unseen_label)
        self.test_in_loader = DataLoader(dataset=test_in_set, batch_size=int(self.batch_size), shuffle=False)
        
        test_out_set = torch.utils.data.TensorDataset(self.test_unknown_feature, torch.zeros(self.test_unknown_feature.size(0)))
        self.test_out_loader = DataLoader(dataset=test_out_set, batch_size=int(self.batch_size), shuffle=False)
        
        test_seen_set = torch.utils.data.TensorDataset(self.test_seen_feature, self.test_seen_label)
        self.test_seen_loader = DataLoader(dataset=test_seen_set, batch_size=int(self.batch_size), shuffle=False)
    ...
    def fit_zsl(self):
        ...
        self.weibull_model = openmax_utils.weibull_tailfitting(self.model, train_loader, self.eta, num_class=self.nclass)
        ...
        return acc, auroc, best_model, fpr95
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            if H > best_H:
                ...
                best_model = copy.deepcopy(self.model.state_dict())
        self.model.load_state_dict(best_model)
        self.weibull_model = openmax_utils.weibull_tailfitting(self.model, train_loader, self.eta, num_class=known_classes.size(0))
        ...
        return acc_seen, acc_unseen, H, epoch, best_model, auroc, fpr95
    ...
    def val_auroc_gzsl(self, test_seen_feature, test_seen_label, test_unseen_feature, test_unseen_label, seenclasses, unseenclasses, test_unknown_feature):
        ...
        seen_modified_score,_=openmax_utils.openmax(self.model, self.test_seen_loader, self.weibull_model, alpha=self.alpha, num_class = self.nclass)
        in_modified_score,_=openmax_utils.openmax(self.model, self.test_in_loader, self.weibull_model, alpha=self.alpha, num_class = self.nclass)
        out_modified_score,_=openmax_utils.openmax(self.model, self.test_out_loader, self.weibull_model, alpha=self.alpha, num_class = self.nclass)
        _, predicted_seen = torch.max(torch.Tensor(seen_modified_score[:, :nclass_seen+nclass_unseen]), 1)
        _, predicted_unseen = torch.max(torch.Tensor(in_modified_score[:, :nclass_seen+nclass_unseen]), 1)
        
        pred_k.extend(seen_modified_score)
        pred_k.extend(in_modified_score)
        pred_u.extend(out_modified_score)
    ...
    def val_auroc(self, test_unseen_feature, test_unseen_label, unseenclasses, test_unknown_feature):
        ...
        in_modified_score,_=openmax_utils.openmax(self.model, self.test_in_loader, self.weibull_model, alpha=self.alpha, num_class = self.nclass)
        out_modified_score,_=openmax_utils.openmax(self.model, self.test_out_loader, self.weibull_model, alpha=self.alpha, num_class = self.nclass)
        _, predicted_known = torch.max(torch.Tensor(in_modified_score[:, :nclass]), 1)
        pred_k.extend(in_modified_score)
        pred_u.extend(out_modified_score)
        # for i in range(0, ntest, self.batch_size):
        #     end = min(ntest, start+self.batch_size)
