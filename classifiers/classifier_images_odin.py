...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, netDec=None, dec_size=4096, dec_hidden_size=4096, usesoftmax = True, test_openset = True, temperature = 1, NoiseMagnitude = 0.014):
    ...
    #         perm_out = torch.randperm(self.ntrain_out)
    #         self.train_X = self.train_X[perm]
    #         self.train_X = self.train_X_out[perm_out]
    ...
    def val_auroc_gzsl(self, test_seen_feature, test_seen_label, test_unseen_feature, test_unseen_label, seenclasses, unseenclasses, test_unknown_feature):
        ...
        for i in range(0, ntest_seen, self.batch_size):
            ...
            inputX = Variable(test_seen_feature[start:end].cuda(), requires_grad = True)
            output, ori_logits = self.model(inputX)
            logits = torch.nn.Softmax(dim=-1)(ori_logits).data.cpu()
            temp_logits = ori_logits / self.temperature
            maxIndexTemp = np.argmax(logits, axis=1)
            labels = Variable(torch.LongTensor(maxIndexTemp).cuda())
            loss = criterion(temp_logits, labels)
            loss.backward()
            gradient = torch.ge(inputX.grad.data, 0)
            gradient = (gradient.float() - 0.5) * 2
            # tempinputX = torch.add(inputX.data, -self.magnitude, gradient)
            tempinputX = inputX.data - self.magnitude * gradient
            _, ori_logits = self.model(tempinputX)
            temp_logits = ori_logits / self.temperature
            logits = torch.nn.Softmax(dim=-1)(temp_logits)
        ...
        for i in range(0, ntest_unseen, self.batch_size):
            ...
            inputX = Variable(test_unseen_feature[start:end].cuda(), requires_grad = True)
            output, ori_logits = self.model(inputX)
            logits = torch.nn.Softmax(dim=-1)(ori_logits).data.cpu()
            temp_logits = ori_logits / self.temperature
            maxIndexTemp = np.argmax(logits, axis=1)
        ...
        for i in range(0, ntest_u, self.batch_size):
            ...
            inputX = Variable(test_unknown_feature[start:end].cuda(), requires_grad = True)
            output, ori_logits = self.model(inputX)
            logits = torch.nn.Softmax(dim=-1)(ori_logits).data.cpu()
            temp_logits = ori_logits / self.temperature
            maxIndexTemp = np.argmax(logits, axis=1)
