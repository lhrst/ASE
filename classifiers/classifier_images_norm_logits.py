...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, netDec=None, dec_size=4096, dec_hidden_size=4096, usesoftmax = True, test_openset = True):
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):      
                ...
                output, logits = self.model(inputv)
                norms = torch.norm(logits, p=2, dim=-1, keepdim=True) + 1e-7
                logit_norm = torch.div(logits, norms)
                loss = F.cross_entropy(logit_norm, labelv)
    ...
    def val_auroc(self, test_unseen_feature, test_unseen_label, unseenclasses, test_unknown_feature):
        ...
        self.pred_k_logits = pred_k
        self.pred_k_labels = labels
        self.pred_u_logits = pred_u
...
class LINEAR_LOGSOFTMAX_CLASSIFIER(nn.Module):
    def __init__(self, input_dim, nclass):
        ...
        self.fc = NormedLinear(input_dim, nclass)
