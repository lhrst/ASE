...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, _train_X_out, _train_Y_out, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, netDec=None, dec_size=4096, dec_hidden_size=4096, usesoftmax = True, test_openset = True, temperature = 1, m_in=-23, m_out=-5):
        self.train_X =  _train_X.clone() 
        self.train_X_out = _train_X_out.clone()
        self.train_Y = _train_Y.clone() 
        self.train_Y_out = _train_Y_out.clone()
        ...
        if self.netDec:
            ...
            self.train_X_out = self.compute_dec_out(self.train_X_out, self.input_dim)
            if test_openset:
        ...
        self.ntrain_out = self.train_X_out.size()[0]
        if generalized:
            if self.test_openset:
                self.acc_seen, self.acc_unseen, self.H, self.epoch, self.best_model, self.auroc= self.fit()
            else:
            ...
            if self.test_openset:
                self.acc, self.auroc, self.best_model = self.fit_zsl() 
            else:
    ...
    def fit_zsl(self):
        ...
        train_set_in = torch.utils.data.TensorDataset(self.train_X, self.train_Y)
        train_set_out = torch.utils.data.TensorDataset(self.train_X_out, self.train_Y_out)
        train_loader_in = DataLoader(dataset=train_set_in, batch_size=int(self.batch_size/2), shuffle=True)
        train_loader_out = DataLoader(dataset=train_set_out, batch_size=int(self.batch_size/2), shuffle=True)
        for epoch in range(self.nepoch):
            train_loader_out.dataset.offset = np.random.randint(len(self.train_Y))
            ...
            if self.test_openset:
                acc, auroc, ROC, Precision, Recall = self.val_auroc(self.test_unseen_feature, self.test_unseen_label, self.unseenclasses, self.test_unknown_feature)
                if acc > best_acc:
                    best_acc = acc
                    best_auroc = auroc
                    best_model = copy.deepcopy(self.model.state_dict())
            ...
            #print('acc %.4f' % (acc))
                ...
                if acc > best_acc:
                    ...
                    best_model = copy.deepcopy(self.model.state_dict())
        if self.test_openset:
            return best_acc, best_auroc, best_model
        else:
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            if self.test_openset:
                acc_seen, acc_unseen, H, auroc, ROC = self.val_auroc_gzsl(self.test_seen_feature, util.map_label(self.test_seen_label, known_classes), \
        ...
        if self.test_openset:
            self.ROC = ROC
            return best_seen, best_unseen, best_H, epoch, best_model, best_auroc
        else:
    ...
    def val_auroc_gzsl(self, test_seen_feature, test_seen_label, test_unseen_feature, test_unseen_label, seenclasses, unseenclasses, test_unknown_feature):
        ...
        return acc_seen, acc_unseen, H, auroc, ROC
    def val_auroc(self, test_unseen_feature, test_unseen_label, unseenclasses, test_unknown_feature):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if logits.size(1) == nclass + 1:
                FP += torch.sum(torch.argmax(logits, 1) == nclass)
        ...
        for i in range(0, ntest_u, self.batch_size):
            ...
            if self.usesoftmax:
                logits = torch.nn.Softmax(dim=-1)(logits)
            if logits.size(1) == nclass + 1:
                TP += torch.sum(torch.argmax(logits, 1) == nclass)
            # logits = logits.data[:, :nclass]
            pred_u.extend(logits.data.cpu().numpy())
        ...
        return acc, auroc, ROC, Precision, Recall
    ...
    def compute_oscr(self, pred_k, pred_u, labels, nclasses):
        if pred_k.shape[1] == nclasses + 1:
            ...
            x1 = self.temperature * logsumexp(pred_k / self.temperature, axis=1)
            x2 = self.temperature * logsumexp(pred_u / self.temperature, axis=1)
