...
import classifiers.classifier_images_placeholder as classifier_placeholder 
import classifiers.classifier_images_energy as classifier_energy
...
if opt.gzsl:
    ...
    gzsl_cls_placeholder = classifier_placeholder.CLASSIFIER(gzsl_X, gzsl_Y, data, known_classes.size(0),opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, temperature = opt.placeholder_temp, alpha1 = opt.alpha1, alpha2 = opt.alpha2)
else:
    zsl_cls_placeholder = classifier_placeholder.CLASSIFIER(unseen_feature, util.map_label(unseen_label, data.unseenclasses), data, data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=False, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, temperature = opt.placeholder_temp, alpha1 = opt.alpha1, alpha2 = opt.alpha2)
if opt.gzsl:
    print("gzsl_cls H is: ", gzsl_cls_placeholder.H)
    print("gzsl_cls acc_seen is: ", gzsl_cls_placeholder.acc_seen)
    print("gzsl_cls acc_unseen is: ", gzsl_cls_placeholder.acc_unseen)
    print("gzsl_cls fpr95 is: ", gzsl_cls_placeholder.fpr95)
    print("gzsl_cls auroc is: ", gzsl_cls_placeholder.auroc)
else:
    print("zsl_cls_placeholder acc : {}".format(zsl_cls_placeholder.acc))
    print("zsl_cls_placeholder fpr95 : {}".format(zsl_cls_placeholder.fpr95))
    print("zsl_cls_placeholder auroc : {}".format(zsl_cls_placeholder.auroc))
