...
if opt.gzsl:
    ...
    gzsl_cls_maxlogit = classifier.CLASSIFIER(gzsl_X, gzsl_Y, data, known_classes.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                50, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096)
else:
    zsl_cls_maxlogit = classifier.CLASSIFIER(unseen_feature, util.map_label(unseen_label, data.unseenclasses), data,
...
if opt.gzsl:
    print("gzsl_cls H is: ", gzsl_cls_maxlogit.H)
    print("gzsl_cls acc_seen is: ", gzsl_cls_maxlogit.acc_seen)
    print("gzsl_cls acc_unseen is: ", gzsl_cls_maxlogit.acc_unseen)
    print("gzsl_cls auroc is: ", gzsl_cls_maxlogit.auroc)
    print("gzsl_cls fpr95 is: ", gzsl_cls_maxlogit.fpr95)
else:
    print("zsl_cls_maxlogit acc : {}".format(zsl_cls_maxlogit.acc))
    print("zsl_cls_maxlogit auroc : {}".format(zsl_cls_maxlogit.auroc))
    print("zsl_cls_maxlogit fpr95 : {}".format(zsl_cls_maxlogit.fpr95))
