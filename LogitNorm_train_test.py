...
import classifiers.classifier_images_norm_logits as classifier_norm_logits 
...
if opt.gzsl:
    ...
    gzsl_cls_norm_logits = classifier_norm_logits.CLASSIFIER(gzsl_X, gzsl_Y, data, known_classes.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=True, usesoftmax = False, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096)
else:
    zsl_cls_norm_logits = classifier_norm_logits.CLASSIFIER(unseen_feature, util.map_label(unseen_label, data.unseenclasses), data, data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=False, usesoftmax = False, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096)

    
if opt.gzsl:
    print("gzsl_cls H is: ", gzsl_cls_norm_logits.H)
    print("gzsl_cls acc_seen is: ", gzsl_cls_norm_logits.acc_seen)
    print("gzsl_cls acc_unseen is: ", gzsl_cls_norm_logits.acc_unseen)
    print("gzsl_cls auroc is: ", gzsl_cls_norm_logits.auroc)
    print("gzsl_cls fpr95 is: ", gzsl_cls_norm_logits.fpr95)
else:
    print("zsl_cls_norm_logits acc : {}".format(zsl_cls_norm_logits.acc))
    print("zsl_cls_norm_logits auroc : {}".format(zsl_cls_norm_logits.auroc))
    print("zsl_cls_norm_logits fpr95 : {}".format(zsl_cls_norm_logits.fpr95))
